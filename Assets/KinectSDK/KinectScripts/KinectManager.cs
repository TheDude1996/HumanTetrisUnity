...
//	public bool NearMode = false;
	...
	public bool Player1Calibrated = false;
	public bool Player2Calibrated = false;
	...
	void Update()
	{
		if(KinectInitialized)
		{
			...
			if(Player1Calibrated)
			{
				...
				foreach(KinectGestures.GestureData gestureData in player1Gestures.ToArray())
				{
							...
							gestureData.progress >= 0.5f)
						{
							if(GetGestureProgress(gestureData.userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								if(HandCursor1 != null)
								{
									...
									if (HandCursor1.GetComponent<SpriteRenderer>() == null) 
									{
								...
								if(ControlMouseCursor)
								{
									Vector3 vCursorPos = HandCursor1.GetComponent<SpriteRenderer>() != null ? HandCursor1.transform.position :
